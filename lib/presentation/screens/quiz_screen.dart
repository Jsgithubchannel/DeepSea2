...
class _QuizScreenState extends State<QuizScreen> with TickerProviderStateMixin {
  ...
  final RxList<Quiz> _quizzes =
      <Quiz>[
        ...
        Quiz(
          ...
          correctAnswer: 2,
          explanation:
              '해파리에 쏘였을 때는 식초를 상처 부위에 바르는 것이 가장 효과적입니다. 식초는 아직 발사되지 않은 자상 세포(nematocysts)의 활성화를 막아줍니다. 담수로 씻거나 상처를 문지르는 것은 독이 더 퍼질 수 있어 위험합니다.',
        ...
        Quiz(
          ...
          options: ['문어해파리', '작은부레관해파리', '상자해파리', '보름달물해파리'],
          correctAnswer: 2,
          explanation:
              '상자해파리(Box jellyfish)는 세계에서 가장 독성이 강한 해양 생물 중 하나로, 그 독은 심각한 통증, 심장 마비, 심지어 사망까지 초래할 수 있습니다.',
          type: QuizType.emergency,
  ...
  void _updateEmergencyQuizTimer() {
    final emergencyQuizzes =
        _quizzes
            .where((q) => q.type == QuizType.emergency && !q.isCompleted)
            .toList();
    ...
    return Scaffold(
      body: Stack(
        children: [
          ...
          Obx(
            () =>
                _userController.showExpNotification
                    ? Positioned(
  ...
  Widget _buildTabBar() {
    return Container(
      ...
      child: TabBar(
        ...
        tabs: [Tab(text: '일일 퀴즈'), Tab(text: '돌발 퀴즈'), Tab(text: '완료한 퀴즈')],
      ),
    );
  }
  ...
  Widget _buildDailyQuizTab() {
    final dailyQuizzes =
        _quizzes.where((q) => q.type == QuizType.daily).toList();
  ...
  Widget _buildQuizCard(
    ...
    void onCardTap() {
      bool isQuizCompleted =
          quiz.isCompleted || _completedQuizzes.any((q) => q.id == quiz.id);
                ...
                children: [
                  ...
                  if (isLocked)
                    ...
                    ElevatedButton.icon(
                      ...
                      icon: Icon(
                        isEmergency ? Icons.warning : Icons.play_arrow,
                      ),
                      label: Text(isEmergency ? '긴급 퀴즈 풀기' : '퀴즈 시작'),
                      style: ElevatedButton.styleFrom(
                        backgroundColor:
                            isEmergency ? Colors.redAccent : Colors.blue,
                        foregroundColor: Colors.white,
                        padding: EdgeInsets.symmetric(
                          horizontal: 20,
                          vertical: 12,
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Icon(icon, color: Colors.white.withOpacity(0.5), size: 50),
          ),
          SizedBox(height: 20),
          Text(
            message,
  ...
  void _startQuiz(Quiz quiz) {
    ...
    bool isQuizCompleted =
        quiz.isCompleted || _completedQuizzes.any((q) => q.id == quiz.id);
    print(
      '퀴즈 완료 여부: isCompleted=${quiz.isCompleted}, _completedQuizzes에 포함=${_completedQuizzes.any((q) => q.id == quiz.id)}',
    );
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                SizedBox(width: 8),
                Text(
                  '정답 보상: ${quiz.type == QuizType.emergency ? ExpConstants.EMERGENCY_QUIZ_CORRECT : ExpConstants.DAILY_QUIZ_CORRECT} EXP',
                  style: TextStyle(
                    fontSize: 14,
                    fontWeight: FontWeight.bold,
  ...
  void _processQuizAnswer(Quiz quiz, String answer) {
    ...
    print(
      '퀴즈 답변 처리 시작: ${quiz.id} (고유ID: $uniqueId) - ${quiz.title} - 타입: ${quiz.type}',
    );
    ...
    if (isCorrect) {
      try {
        ...
        if (index != -1) {
          ...
          print(
            '퀴즈 완료 상태 업데이트: ${updatedQuiz.id} - isCompleted: ${updatedQuiz.isCompleted}',
          );
  ...
  Future<void> _initCompletedQuizzesAsync() async {
    try {
      ...
      final completedStrings =
          await _userController.getCompletedQuizStringsAsync();
